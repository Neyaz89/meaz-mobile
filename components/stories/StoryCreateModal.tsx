...
export type PrivacyOption = 'public' | 'friends' | 'close_friends' | 'custom';
...
interface StoryCreateModalProps {
  ...
  onChangePrivacy: (privacy: PrivacyOption) => void;
  onSubmit: () => void;
  uploading: boolean;
  media: MediaAsset[];
  caption: string;
...
const PRIVACY_OPTIONS: { key: PrivacyOption; label: string; icon: any }[] = [
  { key: 'public', label: 'Public', icon: 'earth' },
  { key: 'friends', label: 'Friends', icon: 'people' },
  { key: 'close_friends', label: 'Close Friends', icon: 'star' },
  { key: 'custom', label: 'Custom', icon: 'lock-closed' },
];
        ...
        <View style={styles.modal}>
          ...
          <View style={styles.privacyRow}>
            {PRIVACY_OPTIONS.map(opt => (
              <TouchableOpacity
                key={opt.key}
                style={[styles.privacyBtn, privacy === opt.key && styles.privacyBtnActive]}
                onPress={() => onChangePrivacy(opt.key)}
                accessibilityLabel={opt.label}
              >
                <Ionicons name={opt.icon} size={18} color={privacy === opt.key ? '#fff' : '#007AFF'} />
                <Text style={[styles.privacyLabel, privacy === opt.key && { color: '#fff' }]}>{opt.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          >
            {uploading ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitBtnText}>Post Story</Text>}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  captionInput: { backgroundColor: '#f2f2f2', borderRadius: 8, padding: 10, fontSize: 15, width: '100%', minHeight: 40, marginBottom: 10 },
  privacyRow: { flexDirection: 'row', justifyContent: 'center', marginBottom: 10 },
  privacyBtn: { flexDirection: 'row', alignItems: 'center', borderRadius: 8, borderWidth: 1, borderColor: '#007AFF', paddingVertical: 6, paddingHorizontal: 12, marginHorizontal: 4, backgroundColor: '#fff' },
  privacyBtnActive: { backgroundColor: '#007AFF', borderColor: '#007AFF' },
  privacyLabel: { color: '#007AFF', fontWeight: 'bold', marginLeft: 6 },
